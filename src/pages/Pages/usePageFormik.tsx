...
export default function usePageFormik({
  openModal,
  editItem,
  avoidUpdateToast,
  _finally,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (editItem?.id) {
          await axios.put(`/page/${editItem.id}`, values);
          !avoidUpdateToast && toast({ message: 'Page Updated!' });
        } else {
          const { data } = await axios.post('/page', {
            ...values,
            slug: slug_generator(values.name),
          });
          if (data) toast({ message: 'New Page Added!' });
        }
      } catch (error) {
        toast({
...
export const PageForms = ({ formik }: { formik: any }) => (
