...
interface Props {
  toggleAddPopupBtn(history: SupplierHistoryT): void;
}

export default function supplierHistoriesTableCells({
  toggleAddPopupBtn,
}: Props) {
  const cells: MuiTableHeader<SupplierHistoryT & { key: 'sl' }>[] = [
    {
      key: 'id',
      label: 'ID',
    },
        ...
        return (
          <>
            <Button
              size="small"
              variant="contained"
              onClick={() => history?.id && toggleAddPopupBtn(history)}
            >
              Paid
            </Button>
